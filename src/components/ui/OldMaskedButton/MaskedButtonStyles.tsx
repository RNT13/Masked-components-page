...
import { ButtonShape, ButtonSize, ButtonState, ButtonVariant } from './MaskedButtonTypes'

interface StyledMaskedButtonProps {
...
const buttonVariants = {
  ...
  secondary: css<StyledMaskedButtonProps>`
  ...
  toggle: css<StyledMaskedButtonProps>`
    background-color: ${({ theme, $state }) => ($state ? theme.colors.baseBlue.base : theme.colors.baseBlack.light20)};
    border: 2px solid ${({ theme, $state }) => ($state ? theme.colors.baseBlue.base : theme.colors.baseBlack.base)};
    width: 44px;
    height: 24px;
    border-radius: 999px;
    position: relative;
    ...
    &::before {
      ...
      top: 50%;
      left: 2px;
      transform: ${({ $state }) => ($state ? 'translate(20px, -50%)' : 'translate(0, -50%)')};
      width: 18px;
      height: 18px;
      border-radius: 50%;
...
const activeStyles = {
  ...
  glass: css`
    background: ${({ theme }) => theme.colors.baseGlass.light04};
    border-color: ${({ theme }) => theme.colors.baseGlass.light08};
    color: ${({ theme }) => theme.colors.baseBlue.light40};
    box-shadow: 2px 2px 2px ${({ theme }) => theme.colors.baseGlass.light08};
  `
}
...
export const StyledButton = styled.button.withConfig({ shouldForwardProp: prop => !prop.startsWith('$') }) <StyledMaskedButtonProps>`
  width: fit-content;
  display: inline-flex;
  align-items: center;
...
export const ButtonContent = styled.span<{ $state: ButtonState }>`
  ...
  align-items: center;
  gap: 8px;
  opacity: ${({ $state }) => ($state === 'loading' ? 0 : 1)};
