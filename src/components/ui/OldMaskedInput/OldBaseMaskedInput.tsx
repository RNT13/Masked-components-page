...
import { ErrorDiv, FileTrigger, MaskedInputContainer, PasswordToggle, PreviewImageDiv, SearchIcon } from './OldMaskedInputStyles'
...
type Variant = 'default' | 'masked' | 'password' | 'textarea' | 'select' | 'file' | 'search'
...
export const BaseMaskedInput = ({
  ...
  const variants: Record<Variant, JSX.Element> = {
    default: <input {...commonProps} type={type} value={value ?? ''} onChange={e => onChange(e.target.value)} />,

    masked: <IMaskInput {...commonProps} mask={mask!} value={String(value ?? '')} onAccept={val => onChange(val)} />,

    password: (
      <>
        <input {...commonProps} type={showPassword ? 'text' : 'password'} value={value ?? ''} onChange={e => onChange(e.target.value)} />
        <PasswordToggle onClick={() => setShowPassword(s => !s)}>{showPassword ? <AiOutlineEyeInvisible /> : <AiOutlineEye />}</PasswordToggle>
      </>
    ),

    textarea: <textarea {...commonProps} value={value ?? ''} onChange={e => onChange(e.target.value)} />,
    ...
    file: (
      <>
        <input type="file" ref={fileInputRef} multiple={multiple} hidden onChange={e => handleFileChange(e.target.files)} />
